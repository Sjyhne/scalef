...
class INRBase(nn.Module):
    ...
    def _encode_and_decode(self, coords, sample_idx, progress=None):
        ...
        coords_flat = projected.reshape(B * H * W, -1)
    ...
    def forward(
        ...
        # `scale_factor` is a legacy argument from older pipelines that performed an
        # in-model area downsample. The current pipeline renders at HR and applies
        # `avg_pool2d` outside the model, so resampling here is intentionally disabled.
