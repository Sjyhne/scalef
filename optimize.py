...
def _hash_level_shape(input_projection):
    if input_projection is None:
        return None, None
    nl = getattr(input_projection, "n_levels", None)
    nf = getattr(input_projection, "n_features_per_level", None)
    return nl, nf


def _build_hash_scale_attention(args, n_levels, n_features_per_level, device):
    mode = str(args.hash_scale_attention).lower().strip()
    if mode in ("none", "", "off", "disabled", "concat"):
        return None
    if n_levels is None or n_features_per_level is None:
        raise ValueError("hash_scale_attention requires a HashGrid input projection.")
    from models.hash_level_scale_attention import HashLevelScaleAttention

    use_coord = bool(args.hash_scale_coord_gate or args.hash_scale_coord_lowfreq_pe)
    return HashLevelScaleAttention(
        n_levels=int(n_levels),
        features_per_level=int(n_features_per_level),
        hidden_dim=int(args.hash_scale_gate_hidden_dim),
        temperature=float(args.hash_scale_temperature),
        mode=mode,
        use_layernorm=True,
        use_coord_in_gate=use_coord,
        lowfreq_coord_pe=bool(args.hash_scale_coord_lowfreq_pe),
    ).to(device)
...
def build_input_projection_decoder_bundle(args, device, *, output_dim=3):
    ...
    hm = str(args.hash_scale_attention).lower().strip()
    if hm not in ("none", "", "off", "disabled", "concat", "softmax", "sigmoid"):
        raise ValueError(f"Unknown --hash_scale_attention {args.hash_scale_attention!r}.")
    ...
    hs = str(args.hash_scale_attention).lower().strip()
    if hs not in ("none", "", "off", "disabled", "concat") and canon != "hashgrid_tcnn":
        raise ValueError("--hash_scale_attention is only valid with HashGrid projections.")
    ...
    nl, nf = _hash_level_shape(input_projection)

    hash_scale_mod = _build_hash_scale_attention(args, nl, nf, device) if canon == "hashgrid_tcnn" else None
    ...
    return input_projection, decoder, nl, nf, hash_scale_mod


def build_inr_model(input_projection, decoder, args, device, *, hash_scale_mod=None, nl=None, nf=None):
    from models.radiometric import FrameRadiometric

    rad = None
    rm = str(getattr(args, "radiometric", "none")).lower().strip()
    if rm in ("gain", "gain_offset"):
        rad = FrameRadiometric(args.num_samples, num_bands=3, mode=rm)
    return get_inr(
        input_projection,
        decoder,
        args.num_samples,
        ...
        hash_scale_attention=hash_scale_mod,
        hash_n_levels=nl,
        hash_n_features_per_level=nf,
        radiometric=rad,
        hash_gate_warmup_iters=int(getattr(args, "hash_scale_warmup_iterations", 0)),
    ).to(device)


def _align_prediction_hwc_to_target(pred: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
    """Resize ``pred`` [B,H,W,C] to match ``target`` [B,Ht,Wt,C] (HR render vs LR supervision)."""
    if pred.shape[1:3] == target.shape[1:3]:
        return pred
    b, ht, wt, _ = target.shape
    x = pred.permute(0, 3, 1, 2).contiguous()
    x = F.interpolate(x, size=(ht, wt), mode="area")
    return x.permute(0, 2, 3, 1).contiguous()


def _reconstruction_loss_fn(args):
    kind = str(getattr(args, "recon_loss", "mse")).lower().strip()
    if kind == "mse":
        return lambda p, t: BasicLosses.mse_loss(p, t)
    if kind == "huber":
        d = float(getattr(args, "huber_delta", 0.03))
        return lambda p, t: BasicLosses.huber_loss(p, t, delta=d)
    if kind in ("charbonnier", "charb"):
        e = float(getattr(args, "charbonnier_eps", 1e-3))
        return lambda p, t: BasicLosses.charbonnier_loss(p, t, epsilon=e)
    raise ValueError(f"Unknown --recon_loss {kind!r}. Use mse, huber, or charbonnier.")
...
def train_one_iteration(
    ...
    *,
    ...
    if use_gnll_loss:
        ...
        recon_criterion = _reconstruction_loss_fn(args)
    ...
    if getattr(args, "progressive_hashgrid", False) and getattr(model, "hash_n_levels", None) is not None:
        progress["progressive"] = True
    progress["hash_gate_warmup_iters"] = int(getattr(args, "hash_scale_warmup_iterations", 0))
    ...
    # INR renders on the HR coordinate grid from the batch; supervision is on LR pixels — pool here.
    if use_gnll_loss:
        output, pred_shifts, pred_variance = model(
            input, sample_id, scale_factor=1, lr_frames=lr_target, progress=progress
        )
        output = _align_prediction_hwc_to_target(output, lr_target)
        if pred_variance is not None:
            pred_variance = _align_prediction_hwc_to_target(pred_variance, lr_target)
        ...
        if variance_reg > 0.0 or variance_smooth_reg > 0.0:
            if hasattr(model, "use_separate_ud") and model.use_separate_ud and hasattr(model, "variances"):
                ...
                for sid in sample_id:
                    log_var = model.variances[sid.item()]
                    log_var_list.append(log_var)

                if log_var_list:
                    log_vars = torch.stack(log_var_list, dim=0)
        ...
        output, pred_shifts = model(input, sample_id, scale_factor=1, lr_frames=lr_target, progress=progress)
        output = _align_prediction_hwc_to_target(output, lr_target)
        if heldout:
    ...
    radio_loss = torch.tensor(0.0, device=device)
    if getattr(model, "radiometric", None) is not None and not heldout:
        g = torch.tanh(model.radiometric.raw_gain)
        radio_loss = float(getattr(args, "radiometric_lambda_gain", 1e-3)) * (g**2).mean()
        if model.radiometric.raw_offset is not None:
            o = torch.tanh(model.radiometric.raw_offset)
            radio_loss = radio_loss + float(getattr(args, "radiometric_lambda_offset", 1e-3)) * (o**2).mean()

    total_loss = recon_loss + variance_reg_loss + variance_smooth_loss + radio_loss
    total_loss.backward()
    optimizer.step()
...
def main():
    ...
    parser.add_argument("--hash_encoding_preset", type=str, default="hashgrid")
    ...
    parser.add_argument(
        ...
        type=str,
        default="none",
        help="none (concat) | softmax | sigmoid — per-level gating on HashGrid features.",
    )
    parser.add_argument("--hash_scale_coord_gate", action="store_true")
    parser.add_argument("--hash_scale_coord_lowfreq_pe", action="store_true")
    parser.add_argument("--hash_scale_gate_hidden_dim", type=int, default=64)
    parser.add_argument("--hash_scale_temperature", type=float, default=1.0)
    parser.add_argument(
        "--hash_scale_warmup_iterations",
        type=int,
        default=0,
        help="For scale gates: first N iterations use identity gates (plan §10.5). 0 disables.",
    )

    parser.add_argument(
        "--progressive_hashgrid",
        action="store_true",
        help="Coarse-to-fine HashGrid level mask (hard schedule; see tto_only_superf_improvement_plan.md §6.3).",
    )
    parser.add_argument(
        "--radiometric",
        type=str,
        default="none",
        choices=["none", "gain", "gain_offset"],
        help="Per-frame per-band radiometric correction on RGB predictions.",
    )
    parser.add_argument("--radiometric_lambda_gain", type=float, default=1e-3)
    parser.add_argument("--radiometric_lambda_offset", type=float, default=1e-3)
    parser.add_argument(
        "--recon_loss",
        type=str,
        default="mse",
        choices=["mse", "huber", "charbonnier"],
        help="Pixel reconstruction loss on downsampled predictions.",
    )
    ...
    if args.multi_sample:
        ...
        input_projection, decoder, nl, nf, hash_scale_mod = build_input_projection_decoder_bundle(
    ...
    metrics_dict = {
        ...
        'model': args.model,
        'input_projection': args.input_projection,
        'progressive_hashgrid': bool(getattr(args, "progressive_hashgrid", False)),
        'radiometric': str(getattr(args, "radiometric", "none")),
        'recon_loss': str(getattr(args, "recon_loss", "mse")),
        'hash_scale_attention': args.hash_scale_attention,
        'hash_scale_coord_gate': bool(args.hash_scale_coord_gate),
        'hash_scale_coord_lowfreq_pe': bool(args.hash_scale_coord_lowfreq_pe),
        'hash_scale_gate_uses_coordinates': bool(
            args.hash_scale_coord_gate or args.hash_scale_coord_lowfreq_pe
        ),
        'hash_scale_attn_stats': getattr(model, "last_hash_scale_stats", None) or {},
        'iterations': args.iters,
