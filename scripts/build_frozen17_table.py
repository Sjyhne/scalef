...
def main() -> None:
    ...
    tex = "\n".join([
        ...
        r"  terminated no run early).}",
        r"  \label{tab:frozen-17}",
        r"  \footnotesize",
        r"  \setlength{\tabcolsep}{3.2pt}",
        r"  \begin{tabular}{lrrr}",
        r"    \toprule",
        r"    Site & LPIPS $\downarrow$ & Bil.\ & Time (s) \\",
        r"    \midrule",
        *rows,
        r"    \midrule",
        f"    Mean $\\pm$ site SD (17) & ${out['lpips_mean']:.3f}\\pm{out['lpips_site_sd']:.3f}$ & "
        f"${out['lpips_bilinear_mean']:.3f}\\pm{out['lpips_bilinear_site_sd']:.3f}$ & "
        f"${out['time_s_mean']:.0f}\\pm{out['time_s_site_sd']:.0f}$ \\\\",
        f"    Mean (seven sites) & {out['seven_site']['lpips']:.3f} & {out['seven_site']['bilinear']:.3f} & "
